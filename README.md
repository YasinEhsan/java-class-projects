...
Simplie Airline Reservation System

The goal of this project is to create a simple simulation of a booking system for a small airline that runs commuter ights between two cities on a daily basis. Your projects should just keep track of ights for the month of October 2017.
...
The Main class contains only static methods (and no instance vari-ables). Its static methods are used to run the simulation and call on the other classes as needed. This should be the last class implemented and should carry out the simulation described below. It is best to start with the other four classes. For each of the classes you should begin by deciding what type to use for each of the instance vari-ables. The next few paragraphs outline possible instance variables and methods for these classes. It is possible to implement the simu-lation using only these variables and methods, however you may use any other choice of variables and methods that allows you to carry out the required simulation.

The class Airline contains information about all the ights for an airline. It uses instance variables name and ights. The variable ights should be an ArrayList of Flight objects. The variable name is a String that gives the name of the airline.

The class Passenger contains information about a passenger. It uses instance variables rstName, lastName, address, phone and myT-ickets. The eld myTickets is an ArrayList of tickets that is used to store all the airline tickets that have been booked by a passenger. The other elds should all be stored as Stings.

The class Ticket contains information about a ticket. It uses instance variables ticketNumber, myAirline, myPassenger, myFlight and price. The class should also have a static data eld called counter which stores an integer that is increased every time a new ticket is issued. This static eld can be used to make sure that ticket numbers are unique (and not shared by two di erent tickets). Ticket numbers are integers and prices have type double.

The class Flight contains information about a ight. It uses in-stance variables ightNumber, seats, lledSeats, ightLength, airline, date, originAirport, destination, departureTime and tickets. The eld tickets should be an ArrayList of tickets that have been issued for the
...
ight. A static eld called counter should be used to keep account of the di erent ights and make sure that each gets a di erent ight number.

For each of these four classes, there is one constructor. The pa-rameters for this constructor should give values for instance variables. However, if the value for an instance variable can be automatically generated no corresponding parameter should be used. So for ex-ample the Flight constructor would not use a parameter to specify lledSeats since this always begins with the value 0. Similarly, the constructor for a Ticket would not have a parameter to specify a ticketNumber since this is automatically generated by using the static eld counter.
...
Several methods are used to nd a convenient ight that matches a choice of date and time. It is unlikely that the time that a passenger chooses here will happen to match the exact time of a scheduled ight, accordingly these methods will return all ights that depart on the correct date within 4 hours of the preferred time. The methods that belong to the four classes have the following title lines and tasks.
...
bank balances for passengers or airlines. ArrayList<Flight> findFlights(String date, double time, String origin)

finds all flights for a 4 hour departure window Ticket book(Passenger p, Flight f)
...
You should create a main method that operates the following sim-ulation of a user's booking experience. Before the user begins book-ing ights at step 4, the simulation needs to create an Airline and its scheduled list of ights. It then creates existing bookings so that some ights will already be full or near to full to simulate the possibility that our desired ight is already sold out. These existing bookings are created at step 3. Do not worry about generating sensible names or other data for the passengers at step 3, their only purpose is to
...
ll seats. The interaction with the user takes place at steps 4 to 7. The user might decide to book several ights here and cancel some of them.
